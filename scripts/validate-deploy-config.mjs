...
const required = ["version", "shareOrigin", "returnOrigin", "registryOrigin", "credentialsOrigin", "emailOrigin", "nodeOrigin", "nodeAudience", "nodeInvitationKid", "nodeInvitationPublicKey", "nodeKeyVersion", "nodeEnabled", "issuerDid", "issuerVct", "issuerKid", "issuerPublicKey", "issuerKeyVersion", "issuerEnabled"];
...
for (const key of ["shareOrigin", "returnOrigin", "registryOrigin", "credentialsOrigin", "emailOrigin", "nodeOrigin"]) { if (typeof value[key] !== "string" || !/^https:\/\/[^/?#:@]+$/.test(value[key])) throw new Error(`SHARE_TRUST_BUNDLE.${key} must be a canonical HTTPS origin`); }
if (process.env.SHARE_UPLOAD_AUTHORIZATION_ORIGIN !== undefined && process.env.SHARE_UPLOAD_AUTHORIZATION_ORIGIN !== value.registryOrigin) throw new Error("SHARE_UPLOAD_AUTHORIZATION_ORIGIN must equal the trusted registryOrigin");
...
if (process.env.SHARE_ACCOUNTLESS_RECEIVER_ENABLED !== undefined && process.env.SHARE_ACCOUNTLESS_RECEIVER_ENABLED !== "true" && process.env.SHARE_ACCOUNTLESS_RECEIVER_ENABLED !== "false") throw new Error("SHARE_ACCOUNTLESS_RECEIVER_ENABLED must be exactly true or false");
