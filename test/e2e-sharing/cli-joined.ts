...
async function main(): Promise<void> {
  ...
  try {
    ...
    const authProbe = await runCommand(process.execPath, ["--input-type=module", "-e", `import { readFile, appendFile } from "node:fs/promises"; import { TinyCloudNode } from ${JSON.stringify(join(install, "node_modules/@tinycloud/node-sdk/dist/index.js"))}; const profile = JSON.parse(await readFile(${JSON.stringify(join(cliHome, ".tinycloud/profiles/joined/profile.json"))}, "utf8")); const session = JSON.parse(await readFile(${JSON.stringify(join(cliHome, ".tinycloud/profiles/joined/session.json"))}, "utf8")); try { const node = new TinyCloudNode({ host: profile.host }); await node.restoreSession({ ...session, verificationMethod: profile.sessionDid }); node.invokeAny([{ spaceId: session.spaceId, service: "capabilities", action: "tinycloud.capabilities/read" }], [{ requestBodyDigest: "probe" }]); await appendFile(${JSON.stringify(nodeStatus)}, "auth-probe=ok\\n"); } catch (error) { await appendFile(${JSON.stringify(nodeStatus)}, "auth-probe=error:" + String(error?.name ?? "unknown") + "\\n"); process.exitCode = 1; }`], install, {}, { allowLoopbackPorts: [] });
    if (authProbe.status !== 0) throw new Error(`packed Node SDK session probe failed: ${(await readFile(nodeStatus, "utf8").catch(() => "unobserved")).replace(/[^A-Za-z0-9=:_\\n-]/g, "")}`);
    const adapterProbe = await runCommand("bun", ["--input-type=module", "-e", `import { appendFile } from "node:fs/promises"; import { createProductionUploadAuthorizer } from ${JSON.stringify(join(sdkWorktree, "packages/cli/src/share/adapters.ts"))}; const fetchFn = async () => { await appendFile(${JSON.stringify(nodeStatus)}, "adapter-fetch=ok\\n"); return new Response("{}", { status: 400 }); }; try { const authorize = createProductionUploadAuthorizer({ fetchFn, profileName: async () => "joined" }); await authorize({ cid: "bafyjoinedprobe", blob: new Uint8Array([1]), contentLength: 1, deleteAfter: new Date(Date.now() + 60000).toISOString() }); await appendFile(${JSON.stringify(nodeStatus)}, "adapter=ok\\n"); } catch (error) { await appendFile(${JSON.stringify(nodeStatus)}, "adapter-error:" + String(error?.name ?? "unknown") + ":" + String(error?.code ?? "none") + "\\n"); }`], sdkWorktree, { HOME: consumerHome, TC_HOME: cliHome, TC_PROFILE: "joined" }, { allowLoopbackPorts: [] });
