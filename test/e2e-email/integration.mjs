...
function spawnOwnedArgs(command, args, cwd, extraEnv = {}) {
  ...
  const collect = (chunk) => {
    ...
    output += text;
    if (text.includes("mounted session:") || text.includes("mounted policy session:") || text.includes("mounted authorize")) console.error(`owned fixture: ${text.trim().slice(0, 600)}`);
