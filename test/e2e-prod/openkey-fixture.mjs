export const OPENKEY_TEST_SESSION_TOKEN = "tinycloud-native-sharing-e2e";
...
addEventListener("message",async(event)=>{
  ...
  if(data?.type==="openkey:auth:request"){
    parent.postMessage({type:"openkey:auth:response",success:true,address:${JSON.stringify(address)},keyId:"tinycloud-native-e2e-key",keyType:"MANAGED",sessionToken:${JSON.stringify(OPENKEY_TEST_SESSION_TOKEN)}},target);
    return;
  }
...
export function openKeyCors(origin) {
  if (origin !== "https://share.tinycloud.xyz") return {};
  return {
