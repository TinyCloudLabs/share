...
export function normalizeFolderPage(value: unknown): FolderPage {
  ...
  const entries = raw.map((entry): FolderEntry => {
    if (typeof entry === "string") {
      if (!safePath(entry)) throw new Error("folder entry is invalid");
    ...
    if (Object.keys(item).some((key) => key !== "path" && key !== "kind") || !safePath(item.path as string) || (item.kind !== undefined && item.kind !== "file" && item.kind !== "folder")) throw new Error("folder entry is invalid");
    return { path: item.path as string, kind: item.kind === "folder" ? "folder" : "file" };
  });
  if (Object.keys(record).some((key) => key !== "entries" && key !== "paths" && key !== "nextCursor") || (record.nextCursor !== null && record.nextCursor !== undefined && typeof record.nextCursor !== "string")) throw new Error("folder response is invalid");
  return { entries, ...(typeof record.nextCursor === "string" ? { nextCursor: record.nextCursor } : {}) };
}

function safePath(path: string): boolean {
  const value = path.replace(/\/$/, "");
  return value.length > 0 && !/[\\\u0000-\u001f\u007f]/.test(value) && value.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== ".." && !/%2f|%5c|%2e/i.test(segment));
}

export function renderFolder(container: HTMLElement, page: readonly FolderEntry[] | FolderPage, prefix: string, pageSize = 50, onNavigate?: (path: string, kind: FolderEntry["kind"]) => void, onPage?: (cursor: string | undefined) => void, onBack?: (path: string) => void): void {
