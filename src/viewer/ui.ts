...
export function renderEmailClaimState(root: HTMLElement, state: import("../email-share/claim.js").ClaimState, actions: EmailClaimViewActions): void {
  ...
  if (state.state === "ready" || state.state === "verifying") { const otp = el(doc, "button", "viewer-secondary-action", "Use email code instead"); otp.type = "button"; otp.addEventListener("click", actions.onUseOtp); box.append(otp); }
