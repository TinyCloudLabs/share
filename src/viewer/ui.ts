...
export function renderEmailClaimState(root: HTMLElement, state: import("../email-share/claim.js").ClaimState, actions: EmailClaimViewActions): void {
  ...
  const detailText = state.state === "claimed" ? "Your browser now holds a non-extractable key for this share. The node will authorize one read using that key." : state.state === "otp" ? (state.message ?? "The link scanner-safe step is complete. Enter the six-digit code from the invitation email.") : state.state === "used" || state.state === "expired" || state.state === "revoked" || state.state === "denied" ? state.message : state.state === "error" ? (state.retryable ? "The service is unavailable. You can retry when it is ready." : "Ask the sender for a fresh invitation.") : state.state === "reading" || state.state === "session" ? "The browser is checking the signed Node response before showing any document bytes." : "The envelope and exact policy are verified locally. Selecting Open document is the required confirmation; simply visiting this link is inert.";
