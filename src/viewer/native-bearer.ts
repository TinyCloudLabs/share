...
export async function receiveNativeBearerAccess(sharing: NativeSharingService, href: string): Promise<{ readonly path: string; readonly bytes: Uint8Array }> {
  ...
  return { path: received.data.path, bytes: read.data.data.slice() };
}
