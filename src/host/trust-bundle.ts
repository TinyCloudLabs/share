...
export function securityHeadersForPath(bundle: ShareTrustBundle, pathname: string): Record<string, string> {
  ...
  const csp = isViewer
    ? `default-src 'none'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src ${connect}; font-src 'self'; frame-src 'self' ${openKeyFrame}; base-uri 'none'; form-action 'self'; object-src 'none'; frame-ancestors 'none'; require-trusted-types-for 'script'; trusted-types share-viewer-html dompurify 'allow-duplicates'`
